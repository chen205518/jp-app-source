...
<template>
  <div class="h-dvh bg-[#7BA05B] flex flex-col overflow-hidden">
    ...
    <div class="flex-1 flex flex-col px-3 gap-y-3 overflow-y-auto pb-3">
      <div class="bg-[#A8C686]/60 backdrop-blur-xl rounded-2xl shadow-lg p-5 border border-white/20 max-w-[100%] mx-auto">
        <div class="grid grid-cols-5 gap-5 w-full">
          <template v-for="(item, rowIndex) in hiraganaData" :key="rowIndex">
            <div
              ...
              :class="[
                'aspect-square rounded-sm flex flex-col items-center justify-center transition-all duration-200 cursor-pointer border border-white/30 backdrop-blur-md relative p-1',
      ...
      <div class="shrink-0">
        <div class="bg-[#6D5D4D]/80 backdrop-blur-xl rounded-xl shadow-lg p-1.5 text-center border border-white/20 max-w-[55%] mx-auto">
          ...
          <div class="text-sm text-[#F5F5DC] mb-1.5">{{ selectedChar?.romanji }}</div>
          <button
            @click="speakJapanese(selectedChar?.hiragana || selectedChar?.katakana || '')"
            :disabled="!selectedChar || isSpeaking"
            class="w-full bg-[#4A6741] hover:bg-[#3A5731] disabled:bg-[#4A6741]/50 text-white rounded-lg py-1.5 px-2.5 flex items-center justify-center gap-1 transition-all active:scale-95 text-sm font-medium"
          >
