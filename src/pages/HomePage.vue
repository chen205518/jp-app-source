...
<template>
  <div class="h-dvh bg-[#7BA05B] flex flex-col overflow-hidden">
    <div class="shrink-0 pt-8 pb-3 px-4">
      <h1 class="text-xl font-bold text-white text-center drop-shadow-lg">
        五十音图
      </h1>
    </div>

    <div class="flex-1 flex flex-col px-3 gap-y-3 overflow-y-auto pb-[80px]">
      <div class="flex-1 bg-[#A8C686]/60 backdrop-blur-xl rounded-2xl shadow-lg p-3 border border-white/20">
        <div class="grid grid-cols-5 gap-2 w-full h-full">
          <template v-for="(item, rowIndex) in hiraganaData" :key="rowIndex">
            <div
              ...
              :class="[
                'aspect-square rounded-lg flex flex-col items-center justify-center transition-all duration-200 cursor-pointer border border-white/30 backdrop-blur-md relative',
            ...
            >
              <div
                ...
                class="absolute inset-0 bg-[#4A6741]/30 flex items-center justify-center animate-pulse rounded-lg"
              >
                <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z"></path>
                </svg>
              </div>
              <span v-if="char || katakanaData[rowIndex].row[colIndex]" class="text-sm font-bold text-[#4A6741] leading-tight">
                {{ char }}<span v-if="char && katakanaData[rowIndex].row[colIndex]" class="text-xs">/</span>{{ katakanaData[rowIndex].row[colIndex] }}
              </span>
      ...
      <div class="shrink-0">
        <div class="bg-[#6D5D4D]/80 backdrop-blur-xl rounded-xl shadow-lg p-4 text-center border border-white/20">
          <div class="text-xl font-bold text-[#F5F5DC] mb-2">
            {{ selectedChar?.hiragana || selectedChar?.katakana }}
          </div>
          <div class="text-xs text-[#F5F5DC]/80 mb-2">
            {{ selectedChar?.hiragana || '—' }} / {{ selectedChar?.katakana || '—' }}
          </div>
          <div class="text-base text-[#F5F5DC] mb-3">{{ selectedChar?.romanji }}</div>
          <button
            @click="speakJapanese(selectedChar?.hiragana || selectedChar?.katakana || '')"
            :disabled="!selectedChar || isSpeaking"
            class="w-full bg-[#4A6741] hover:bg-[#3A5731] disabled:bg-[#4A6741]/50 text-white rounded-lg py-2.5 px-4 flex items-center justify-center gap-2 transition-all active:scale-95 text-sm font-medium"
          >
